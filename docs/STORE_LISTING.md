...
5. ~~**Firebase for iOS.**~~ — **Done.** `lib/firebase_options.dart`'s iOS
   block, `PRODUCT_BUNDLE_IDENTIFIER` alignment and `CFBundleDisplayName`
   were already fixed in an earlier session (see git history). This session
   found and fixed the remaining real gap: `GoogleService-Info.plist`
   existed on disk but was never wired into the Xcode project (no
   `PBXFileReference`/`PBXBuildFile`/`PBXResourcesBuildPhase` entry), so it
   was never copied into the built `.app` and native Firebase init failed
   with "Could not locate configuration file". Fixed by adding the missing
   project entries; verified via `xcrun simctl spawn ... log stream` that
   Firebase now finds the plist and `main.dart`'s mock-fallback path is not
   taken. Also bumped `IPHONEOS_DEPLOYMENT_TARGET` 12.0 → 13.0 (Podfile +
   all 3 Xcode configs) — `ar_flutter_plugin_2`'s podspec requires iOS 13,
   and `pod install` failed outright without this. Push Notifications
   capability added: `ios/Runner/Runner.entitlements`
   (`aps-environment=development`) + `UIBackgroundModes=remote-notification`
   in Info.plist. `flutter build ios --simulator` succeeds and the app
   launches cleanly on an iPhone 17 Pro simulator. Not verified: a real
   signed build registering with APNs (needs a paid Apple Developer
