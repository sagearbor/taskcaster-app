                ...
                setOnGestureListener(
                    onSingleTapConfirmed = { motionEvent: MotionEvent, node: Node? ->
                        if (node != null) {
                            ...
                            // PATCH (taskcaster): the upstream handler only resolves
                            // tapped names from anchorNodesMap (plane-anchored nodes),
                            // so a tap on a regular addNode node reported null and the
                            // app could never tell which object was tapped. Also walk
                            // the parent chain against nodesMap so free-floating nodes
                            // (our balloons/bombs) report their real name and become
                            // tappable/poppable.
                            if (anchorName == null) {
                                var freeNode: Node? = node
                                while (freeNode != null) {
                                    nodesMap.forEach { (name, n) ->
                                        if (freeNode == n) {
                                            anchorName = name
                                            return@forEach
                                        }
